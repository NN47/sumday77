...
- Не используй Markdown-разметку (`**`, `__`, `#`, `*` и т.п.) ни при каких обстоятельствах.
- Для выделения важных элементов используй только HTML-теги Telegram, например <b>Общая оценка — 9/10</b>, <b>Калории</b>, <b>Белок</b>.
...
async def remind_evening_activity_analysis_later(callback: CallbackQuery):
    ...
    user_id = str(callback.from_user.id)
    target_date = date.fromisoformat(callback.data.split(":", 1)[1])
    if EveningAnalysisNotificationRepository.has_analysis_for_date(user_id, target_date):
        await callback.answer()
        return
