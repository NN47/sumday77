...
async def _return_to_add_methods_from_method_input(
    ...
    *,
    ...
    if data.get("dish_add_destination") and data.get("dish_edit_id"):
        ...
        # Clear the nested input FSM first, then restore only the dish editor
        # context so a later ordinary message cannot be consumed by it.
        await state.clear()
        dish = DishRepository.get_by_id(user_id or str(message.from_user.id), dish_id)
        if dish is None:
...
def _build_saved_dish_card_keyboard(
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="✏️ Редактировать состав", callback_data=f"my_dish_edit:{dish_id}")],
...
PORTION_FRACTIONS = (("quarter", "¼ блюда", 0.25), ("third", "⅓ блюда", 1 / 3),
                     ("half", "½ блюда", 0.5), ("whole", "Всё блюдо", 1.0))


def _format_saved_dish_portion_picker(dish, items: list[dict]) -> str:
    return "\n".join([
        "🍽 <b>Сколько добавить?</b>", "", f"🥣 <b>{html.escape(dish.name)}</b>",
        f"Всё блюдо: <b>{calculate_dish_weight(items):.0f} г</b>", "", "Выбери размер порции:",
    ])


def _build_saved_dish_portion_keyboard(dish_id: int, items: list[dict]) -> InlineKeyboardMarkup:
    total = calculate_dish_weight(items)
    buttons = [InlineKeyboardButton(
        text=f"{label} · {total * factor:.0f} г",
        callback_data=f"my_dish_portion:{dish_id}:{key}",
    ) for key, label, factor in PORTION_FRACTIONS]
    return InlineKeyboardMarkup(inline_keyboard=[
        buttons[:2], buttons[2:],
        [InlineKeyboardButton(text="⚖️ Указать вес вручную", callback_data=f"my_dish_portion_manual:{dish_id}")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"my_dish_portion_back:{dish_id}")],
    ])


def _format_saved_dish_portion_confirmation(dish, items: list[dict]) -> str:
    totals = calculate_dish_totals(items)
    return "\n".join([
        f"🍽 <b>Порция: {html.escape(dish.name)}</b>", "",
        f"📦 <b>Вес:</b> {calculate_dish_weight(items):.0f} г",
        f"🔥 <b>Калории:</b> {totals['calories']:.0f} ккал",
        f"🥩 <b>Белки:</b> {totals['protein']:.1f} г",
        f"🥑 <b>Жиры:</b> {totals['fat']:.1f} г",
        f"🍚 <b>Углеводы:</b> {totals['carbs']:.1f} г",
...
def _build_saved_dish_portion_confirmation_keyboard(
    dish_id: int, meal_type: str, *, choose_meal_type: bool,
) -> InlineKeyboardMarkup:
    action_text = ("✅ Выбрать приём пищи" if choose_meal_type else
                   f"✅ Добавить в {display_meal_type(meal_type).lower()}")
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=action_text, callback_data=f"my_dish_portion_confirm:{dish_id}")],
        [InlineKeyboardButton(text="✏️ Изменить порцию", callback_data=f"my_dish_portion_change:{dish_id}")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"my_dish_portion_back:{dish_id}")],
    ])
...
def _build_saved_dish_editor_keyboard(dish_id: int, items: list[dict]) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text="✏️ Изменить название", callback_data=f"my_dish_rename:{dish_id}")],
        [InlineKeyboardButton(text="➕ Добавить продукт", callback_data=f"my_dish_product_add:{dish_id}")],
    ]
    rows.extend([
        InlineKeyboardButton(
            text=f"{_format_emoji_number(index)} {_truncate_product_name(str(item.get('name') or 'Ингредиент'))}",
...
async def _show_saved_dish_editor(message: Message, state: FSMContext, user_id: str, dish_id: int) -> bool:
    ...
    await state.set_state(MealEntryStates.choosing_meal_type)
    await state.update_data(
        dish_edit_mode=True, dish_edit_id=dish_id, dish_edit_ingredient_id=None,
...
@router.callback_query(lambda c: c.data.startswith("my_dish_edit:"))
async def my_dish_edit_open(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    dish_id = int(callback.data.split(":", 1)[1])
    if not await _show_saved_dish_editor(callback.message, state, str(callback.from_user.id), dish_id):
        await callback.answer("Блюдо не найдено", show_alert=True)


@router.callback_query(lambda c: c.data.startswith("my_dish_ingredient:"))
async def my_dish_ingredient_open(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    _, raw_dish_id, raw_index = callback.data.split(":")
    dish_id, product_idx = int(raw_dish_id), int(raw_index)
    dish = DishRepository.get_by_id(str(callback.from_user.id), dish_id)
    items = dish_to_snapshot(dish) if dish else []
    ...
    await state.update_data(
        dish_edit_mode=True, dish_edit_id=dish_id, dish_edit_ingredient_id=product_idx,
        saved_products=items, weight_drafts={}, kbju_drafts={}, editing_product_idx=product_idx,
    )
    await callback.message.edit_text(
        _render_product_actions_text(items[product_idx]),
        reply_markup=_build_product_actions_keyboard(product_idx), parse_mode="HTML",
    )


@router.callback_query(lambda c: c.data.startswith("my_dish_rename:"))
async def my_dish_rename_start(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    dish_id = int(callback.data.split(":", 1)[1])
    await state.set_state(MealEntryStates.editing_saved_dish_name_input)
    await state.update_data(dish_edit_mode=True, dish_edit_id=dish_id)
    await callback.message.answer(
        "Введите новое название блюда:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="⬅️ Назад", callback_data=f"my_dish_rename_back:{dish_id}")
        ]]),
    )


@router.message(MealEntryStates.editing_saved_dish_name_input)
async def my_dish_rename_apply(message: Message, state: FSMContext):
    name = re.sub(r"\s+", " ", (message.text or "").strip())
    data = await state.get_data()
    ...
    if len(name) > MAX_DISH_NAME_LENGTH:
        await message.answer(f"Название слишком длинное. Введите до {MAX_DISH_NAME_LENGTH} символов.")
        return
    if await _reject_sensitive_food_name(message, name, context="saved_dish_rename"):
        return
    dish = DishService.rename(user_id=str(message.from_user.id), dish_id=dish_id, name=name)
    if dish is None:
        await state.clear()
        await message.answer("❌ Не удалось изменить название блюда.")
        return
    await message.answer("✅ Название блюда изменено")
    # A new message is required after free text; subsequent navigation remains inline.
    await state.set_state(MealEntryStates.choosing_meal_type)
    items = dish_to_snapshot(dish)
...
@router.callback_query(lambda c: c.data.startswith("my_dish_rename_back:"))
async def my_dish_rename_back(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
...
async def my_dish_edit_done(callback: CallbackQuery, state: FSMContext):
    ...
    navigation = {
        ...
        "recipe_catalog": bool(data.get("recipe_catalog")),
        "my_dish_id": dish_id,
        "my_dish_items": items,
        "my_dish_original_items": items,
...
@router.callback_query(lambda c: c.data.startswith("my_dish_product_add:"))
async def my_dish_product_add(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    dish_id = int(callback.data.rsplit(":", 1)[1])
...
async def my_dish_add_method(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    method = callback.data.split(":", 1)[1]
    data = await state.get_data()
    await state.update_data(dish_add_destination=True)
    if method == "text":
        await kbju_add_via_ai(callback.message, state)
    elif method == "label":
        await kbju_add_via_label(callback.message, state)
    elif method == "manual":
        await kbju_add_via_custom_product(callback.message, state)
    else:
        await _show_my_products_page(callback.message, state, meal_type=normalize_meal_type(data.get("meal_type"), fallback=MealType.SNACK.value), page=1, user_id=str(callback.from_user.id), edit_message=True, back_callback_data=f"my_dish_edit:{int(data.get('dish_edit_id') or 0)}")


async def _show_my_dish_portion_picker(message: Message, state: FSMContext, user_id: str, dish_id: int) -> bool:
    data = await state.get_data()
    dish = DishRepository.get_by_id(user_id, dish_id)
    original = data.get("my_dish_original_items") or (dish_to_snapshot(dish) if dish else [])
    if dish is None or not original:
        return False
    await state.set_state(MealEntryStates.choosing_meal_type)
    await state.update_data(
        my_dish_original_items=[dict(item) for item in original],
        my_dish_portion_items=None,
    )
    sender = message.answer if data.get("newly_created_dish") else message.edit_text
    await sender(
        _format_saved_dish_portion_picker(dish, original),
        reply_markup=_build_saved_dish_portion_keyboard(dish_id, original), parse_mode="HTML",
    )
    return True


async def _show_my_dish_portion_confirmation(
    message: Message, state: FSMContext, user_id: str, dish_id: int, items: list[dict],
) -> bool:
    dish = DishRepository.get_by_id(user_id, dish_id)
    ...
    data = await state.get_data()
    await state.set_state(MealEntryStates.choosing_meal_type)
    await state.update_data(my_dish_portion_items=items)
    meal_type = normalize_meal_type(data.get("meal_type"), fallback=MealType.SNACK.value)
    await message.edit_text(
        _format_saved_dish_portion_confirmation(dish, items),
        reply_markup=_build_saved_dish_portion_confirmation_keyboard(
            dish_id, meal_type, choose_meal_type=bool(data.get("recipe_catalog")),
        ), parse_mode="HTML",
    )
    return True
...
async def my_dish_add(callback: CallbackQuery, state: FSMContext):
    ...
    if data.get("dish_builder") or data.get("dish_add_destination"):
        ...
        return
    await callback.answer()
    if not await _show_my_dish_portion_picker(callback.message, state, str(callback.from_user.id), dish_id):
        await callback.answer("Блюдо не найдено", show_alert=True)


@router.callback_query(lambda c: c.data.startswith("my_dish_portion:") and c.data.count(":") == 2)
async def my_dish_portion_select(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    _, raw_dish_id, fraction_key = callback.data.split(":")
    dish_id = int(raw_dish_id)
    data = await state.get_data()
    if dish_id != int(data.get("my_dish_id") or 0):
        await callback.answer(STALE_MEAL_SAVE_TEXT, show_alert=True)
        return
    original = data.get("my_dish_original_items") or []
    factors = {key: factor for key, _label, factor in PORTION_FRACTIONS}
    factor = factors.get(fraction_key)
    if not original or factor is None:
        await callback.answer("Блюдо не найдено", show_alert=True)
        return
    portion = scale_dish_snapshot(original, calculate_dish_weight(original) * factor)
    await _show_my_dish_portion_confirmation(
        callback.message, state, str(callback.from_user.id), dish_id, portion,
    )


@router.callback_query(lambda c: c.data.startswith("my_dish_portion_manual:"))
async def my_dish_portion_manual_start(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    dish_id = int(callback.data.rsplit(":", 1)[1])
    data = await state.get_data()
    if dish_id != int(data.get("my_dish_id") or 0) or not data.get("my_dish_original_items"):
        await callback.answer(STALE_MEAL_SAVE_TEXT, show_alert=True)
        return
    await state.set_state(MealEntryStates.editing_saved_dish_portion_weight_input)
    await callback.message.answer(
        "Введи вес порции в граммах, например: 350",
        reply_markup=_build_photo_analysis_cancel_menu(),
    )


@router.message(MealEntryStates.editing_saved_dish_portion_weight_input)
async def my_dish_portion_manual_apply(message: Message, state: FSMContext):
    data = await state.get_data()
    if data.get("diary_dish_portion_edit"):
        ...
        if not math.isfinite(new_weight) or new_weight < 1:
            await message.answer("Вес должен быть конечным положительным числом не меньше 1 г.")
            return
        items = scale_dish_snapshot(data.get("saved_products") or [], new_weight)
        meal_id = int(data.get("diary_dish_meal_id") or 0)
        totals, api_details = _build_meal_update_payload(items)
        meal = MealRepository.get_meal_by_id(meal_id, str(message.from_user.id))
        if meal is None or not MealRepository.update_meal(
            meal_id=meal_id, user_id=str(message.from_user.id), description=meal.raw_query,
            calories=totals["calories"], protein=totals["protein_g"],
            fat=totals["fat_total_g"], carbs=totals["carbohydrates_total_g"],
            products_json=json.dumps(items, ensure_ascii=False), api_details=api_details,
            is_manually_corrected=any(bool(item.get("is_manually_corrected")) for item in items),
        ):
            await message.answer("Не удалось изменить порцию.")
            return
        await state.set_state(MealEntryStates.editing_meal_weight)
        await state.update_data(saved_products=[{**item, "_source_meal_id": meal_id} for item in items],
                                diary_dish_portion_edit=False)
        await message.answer("✅ Порция блюда обновлена.")
        await _return_to_meal_editor_top(
            message, state, await state.get_data(), str(message.from_user.id),
        )
        return
    ...
    if raw == "❌ Отмена":
        ...
        original = data.get("my_dish_original_items") or []
        if dish is not None and original:
            await message.answer(
                _format_saved_dish_portion_picker(dish, original),
                reply_markup=_build_saved_dish_portion_keyboard(dish_id, original), parse_mode="HTML",
            )
    ...
    if not original:
        await message.answer("Блюдо не найдено.")
        return
    portion = scale_dish_snapshot(original, new_weight)
    dish = DishRepository.get_by_id(str(message.from_user.id), dish_id)
    if dish is None:
        await message.answer("Блюдо не найдено.")
        return
    await state.set_state(MealEntryStates.choosing_meal_type)
    await state.update_data(my_dish_portion_items=portion)
    await message.answer(
        _format_saved_dish_portion_confirmation(dish, portion),
        reply_markup=_build_saved_dish_portion_confirmation_keyboard(
            dish_id, normalize_meal_type(data.get("meal_type"), fallback=MealType.SNACK.value),
            choose_meal_type=bool(data.get("recipe_catalog")),
        ), parse_mode="HTML",
    )


@router.callback_query(lambda c: c.data.startswith("my_dish_portion_change:"))
async def my_dish_portion_change(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    dish_id = int(callback.data.rsplit(":", 1)[1])
    if not await _show_my_dish_portion_picker(callback.message, state, str(callback.from_user.id), dish_id):
        await callback.answer("Блюдо не найдено", show_alert=True)
...
@router.callback_query(lambda c: c.data.startswith("my_dish_portion_back:"))
async def my_dish_portion_back(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    dish_id = int(callback.data.rsplit(":", 1)[1])
    data = await state.get_data()
    dish = DishRepository.get_by_id(str(callback.from_user.id), dish_id)
    ...
    if dish is None or not original:
        await callback.answer("Блюдо не найдено", show_alert=True)
        return
    await state.update_data(my_dish_portion_items=None)
    await callback.message.edit_text(
        _format_saved_dish_card(dish, original),
        reply_markup=_build_saved_dish_card_keyboard(
            normalize_meal_type(data.get("meal_type"), fallback=MealType.SNACK.value),
            int(data.get("my_dishes_page") or 1), dish_id,
...
@router.callback_query(lambda c: c.data.startswith("my_dish_portion_confirm:"))
async def my_dish_portion_confirm(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    dish_id = int(callback.data.rsplit(":", 1)[1])
    token = data.get("my_dish_save_token")
    if (not _is_valid_meal_save_token(token) or dish_id != int(data.get("my_dish_id") or 0)
            or not data.get("my_dish_portion_items")):
        await callback.answer(STALE_MEAL_SAVE_TEXT, show_alert=True)
        return
...
async def my_dish_add_to_meal(callback: CallbackQuery, state: FSMContext):
    """Save a selected recipe portion after choosing its diary destination."""
    data = await state.get_data()
...
async def _save_my_dish_to_diary(
    callback: CallbackQuery, state: FSMContext, data: dict, *, dish_id: int,
) -> None:
    """Shared final save for a confirmed portion selected from any catalog."""
    token = data.get("my_dish_save_token")
    meal_type = normalize_meal_type(data.get("meal_type"), fallback=MealType.SNACK.value)
    try:
    ...
    result = DishService.add_saved_dish_to_diary(
        save_token=token, user_id=str(callback.from_user.id), dish_id=dish_id,
        entry_date=entry_date, meal_type=meal_type, items=data.get("my_dish_portion_items"),
    )
    ...
    await state.update_data(
        my_dish_save_token=None, my_dish_items=None, my_dish_original_items=None,
        my_dish_portion_items=None, in_my_dishes_section=False,
    )
    await _keep_meal_entry_open_after_save(
        callback.message, state, user_id=str(callback.from_user.id),
        entry_date=entry_date, meal_type=meal_type,
    )
...
async def _save_composed_dish(message: Message, state: FSMContext, name: str, *, user_id: str | None = None) -> None:
    ...
    try:
        ...
        entry_date = date.today()
    result = DishService.create_dish_template(
        save_token=token, user_id=resolved_user_id, dish_name=clean, items=items,
        source="manual_composition",
    )
    if result.status is MealSaveStatus.FAILED or result.dish is None:
        await message.answer("Не удалось сохранить блюдо. Черновик остался доступен — попробуй ещё раз."); return
    totals = calculate_dish_totals(items)
    dish_id = result.dish.id
    await state.clear()
    await state.update_data(
        meal_type=normalize_meal_type(builder.get("meal_type"), fallback=MealType.SNACK.value),
        entry_date=entry_date.isoformat(), my_dishes_return_entry_date=entry_date.isoformat(),
        my_dish_id=dish_id, my_dish_items=[dict(item) for item in items],
        my_dish_original_items=[dict(item) for item in items],
        my_dish_save_token=_new_meal_save_token(), newly_created_dish=True,
    )
    await message.answer(
        (f"🎉 <b>Блюдо «{html.escape(result.dish.name)}» создано!</b>\n\n"
         f"Полное блюдо: <b>{calculate_dish_weight(items):.0f} г</b>\n"
         f"{totals['calories']:.0f} ккал (Б {totals['protein']:.1f} / Ж {totals['fat']:.1f} / У {totals['carbs']:.1f})\n\n"
         "Теперь выбери, сколько ты съел:"),
        parse_mode="HTML",
    )
    # Reuse the same picker and scaling path as recipes and saved dishes.
    await _show_my_dish_portion_picker(message, state, resolved_user_id, dish_id)
...
async def edit_last_meal(message: Message, state: FSMContext):
    ...
    products = _meal_editor_items([meal])
    ...
    await state.update_data(
        ...
        saved_products=products,
        editor_items=products,
        grouped_meal_ids=[last_meal_id],
        weight_drafts={},
        kbju_drafts={},
...
def _strip_source_meta(product: dict) -> dict:
    return {k: v for k, v in product.items()
            if not str(k).startswith("_source_") and k not in {
                "kind", "meal_id", "product_index", "_editor_kind",
            }}


def _meal_editor_items(meals: list) -> list[dict]:
    """Build the structural top level of a meal editor.

    A dish is deliberately one item here. Its ingredient snapshot is exposed
    only after the user opens that item.
    """
    result: list[dict] = []
    for meal in meals:
        ...
        if getattr(meal, "entry_kind", "products") == "dish":
            result.append({
                "kind": "dish", "meal_id": meal.id, "dish_id": getattr(meal, "dish_id", None),
                "name": (getattr(meal, "dish_name_snapshot", None)
                         or getattr(meal, "description", None)
                         or getattr(meal, "raw_query", None) or "Блюдо"),
                "grams": calculate_dish_weight(products),
                "kcal": float(getattr(meal, "calories", 0) or 0),
                "calories": float(getattr(meal, "calories", 0) or 0),
                "protein": float(getattr(meal, "protein", 0) or 0),
                "fat": float(getattr(meal, "fat", 0) or 0),
                "carbs": float(getattr(meal, "carbs", 0) or 0),
                "_source_meal_id": meal.id, "_editor_kind": "dish",
            })
            continue
        for index, product in enumerate(products):
            result.append({**product, "kind": "product", "meal_id": meal.id,
                           "product_index": index, "_source_meal_id": meal.id,
                           "_editor_kind": "product"})
    return result
...
async def edit_meal_from_diary_block(callback: CallbackQuery, state: FSMContext):
    ...
    merged_products = _meal_editor_items(meals_for_type)
...
async def meal_weight_back_to_products(callback: CallbackQuery, state: FSMContext):
    ...
    saved_products = data.get("saved_products", [])
    await state.set_state(MealEntryStates.editing_meal_weight)
    if data.get("dish_edit_mode"):
        await _show_saved_dish_editor(callback.message, state, str(callback.from_user.id), int(data.get("dish_edit_id") or 0))
        return
    if data.get("diary_dish_edit_mode"):
        await _return_to_meal_editor_top(
            callback.message, state, data, str(callback.from_user.id),
        )
        return
    if data.get("ai_text_draft_mode"):
        await callback.message.edit_text(
...
async def meal_weight_select_product(callback: CallbackQuery, state: FSMContext):
    ...
    if product.get("_editor_kind") == "dish":
        meal_id = int(product.get("_source_meal_id") or 0)
        meal = MealRepository.get_meal_by_id(meal_id, str(callback.from_user.id))
        ingredients = _extract_products_for_edit(meal) if meal else []
        if not ingredients:
            await callback.answer("Не удалось открыть состав", show_alert=True)
            return
        ingredients = [{**item, "_source_meal_id": meal_id} for item in ingredients]
        await state.update_data(
            saved_products=ingredients, diary_dish_edit_mode=True,
            diary_dish_meal_id=meal_id, diary_dish_name=product.get("name"),
            editing_product_idx=None, weight_drafts={}, kbju_drafts={},
        )
        text = "\n".join([
            f"✏️ <b>{html.escape(str(product.get('name') or 'Блюдо'))}</b>", "",
            f"Порция в этом приёме пищи: <b>{calculate_dish_weight(ingredients):.0f} г</b>", "",
            *[format_product_detail_block(item) for item in ingredients],
        ])
        keyboard = _build_weight_products_keyboard(ingredients)
        keyboard.inline_keyboard.insert(-1, [InlineKeyboardButton(
            text="⚖️ Изменить порцию", callback_data="diary_dish_portion",
        )])
        keyboard.inline_keyboard[-1] = [InlineKeyboardButton(
            text="⬅️ Назад", callback_data="diary_dish_back",
        )]
...
async def _return_to_meal_editor_top(
    message: Message, state: FSMContext, data: dict, user_id: str,
) -> None:
    records = [
        MealRepository.get_meal_by_id(int(meal_id), user_id)
        for meal_id in data.get("grouped_meal_ids") or []
    ]
    items = _meal_editor_items([meal for meal in records if meal is not None])
    if not items:
        items = data.get("editor_items") or []
    await state.update_data(saved_products=items, editor_items=items, diary_dish_edit_mode=False,
                            diary_dish_meal_id=None, editing_product_idx=None,
                            weight_drafts={}, kbju_drafts={})
    await _show_meal_edit_products_list(
        message, items,
        normalize_meal_type(data.get("editing_meal_type") or data.get("grouped_meal_type"),
                            fallback=MealType.SNACK.value),
        edit_existing=True,
    )


@router.callback_query(lambda c: c.data == "diary_dish_back")
async def diary_dish_back(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await _return_to_meal_editor_top(
        callback.message, state, await state.get_data(), str(callback.from_user.id),
    )


@router.callback_query(lambda c: c.data == "diary_dish_portion")
async def diary_dish_portion_start(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    data = await state.get_data()
    if not data.get("diary_dish_edit_mode") or not data.get("saved_products"):
        await callback.answer("Блюдо не найдено", show_alert=True)
        return
    await state.set_state(MealEntryStates.editing_saved_dish_portion_weight_input)
    await state.update_data(diary_dish_portion_edit=True)
    await callback.message.answer(
        "Введи новый вес порции в граммах, например: 250",
        reply_markup=_build_photo_analysis_cancel_menu(),
    )
...
async def meal_product_name_input_value(message: Message, state: FSMContext):
    ...
    if data.get("dish_edit_mode"):
        dish = DishService.replace_ingredients(
            user_id=str(message.from_user.id), dish_id=int(data.get("dish_edit_id") or 0),
            items=saved_products,
        )
        if dish is None:
            await state.clear()
            await message.answer("❌ Не удалось обновить ингредиент блюда.")
            return
        await state.set_state(MealEntryStates.editing_meal_weight)
        await state.update_data(saved_products=dish_to_snapshot(dish), my_dish_items=dish_to_snapshot(dish))
...
async def _save_kbju_changes_for_product(
    ...
    if data.get("dish_edit_mode"):
        dish = DishService.replace_ingredients(
            user_id=user_id, dish_id=int(data.get("dish_edit_id") or 0), items=saved_products,
        )
        if dish is None:
            await callback.answer("Не удалось обновить блюдо", show_alert=True)
            return False
        kbju_drafts.pop(str(product_idx), None)
        await state.set_state(MealEntryStates.edit_kbju_menu)
        await state.update_data(saved_products=dish_to_snapshot(dish), my_dish_items=dish_to_snapshot(dish), kbju_drafts=kbju_drafts)
        await callback.message.edit_text(_render_kbju_editor_text(saved_products[product_idx]), reply_markup=_build_kbju_editor_keyboard(product_idx))
...
async def meal_weight_save(callback: CallbackQuery, state: FSMContext):
    ...
    if data.get("dish_edit_mode"):
        dish = DishService.replace_ingredients(user_id=user_id, dish_id=int(data.get("dish_edit_id") or 0), items=saved_products)
        if dish is None:
            await callback.answer("Не удалось обновить блюдо", show_alert=True)
            return
        drafts.pop(str(product_idx), None)
        fresh = dish_to_snapshot(dish)
        await state.update_data(saved_products=fresh, my_dish_items=fresh, weight_drafts=drafts)
        await callback.answer("✅ Вес ингредиента обновлён")
...
async def meal_weight_delete(callback: CallbackQuery, state: FSMContext):
    ...
    if saved_products[product_idx].get("_editor_kind") == "dish":
        if not MealRepository.delete_meal(source_meal_id, user_id):
            await callback.answer("Не удалось удалить блюдо", show_alert=True)
            return
        saved_products.pop(product_idx)
        await state.update_data(saved_products=saved_products, editor_items=saved_products,
                                editing_product_idx=None)
        if saved_products:
            await _show_meal_edit_products_list(
                callback.message, saved_products,
                normalize_meal_type(data.get("editing_meal_type") or data.get("grouped_meal_type"),
                                    fallback=MealType.SNACK.value), edit_existing=True,
            )
        else:
            await state.clear()
            await callback.message.edit_text("Приём пищи теперь пуст. Запись удалена.")
        return
    ...
    if data.get("dish_edit_mode"):
        ...
        if dish is None:
            ...
            return
        fresh = dish_to_snapshot(dish)
        await state.update_data(saved_products=fresh, my_dish_items=fresh, weight_drafts=drafts, dish_edit_ingredient_id=None)
        await callback.message.edit_text(_format_saved_dish_editor(dish, fresh), reply_markup=_build_saved_dish_editor_keyboard(dish.id, fresh), parse_mode="HTML")
