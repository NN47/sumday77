...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Ответьте на test@example.com", SensitiveDataType.EMAIL),
        ("Мой телефон +7 999 123-45-67", SensitiveDataType.PHONE),
        ("ФИО: Иванов Петр Иванович", SensitiveDataType.PERSONAL_IDENTITY),
        ("ИВАНОВ ИВАН ИВАНОВИЧ", SensitiveDataType.PERSONAL_IDENTITY),
        ("Живу по адресу Москва, ул. Пушкина, д. 12", SensitiveDataType.ADDRESS),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "После обновления не открывается раздел заметок",
        "Слово пароль отображается дважды",
        "Кнопка «Назад» не работает после обновления",
