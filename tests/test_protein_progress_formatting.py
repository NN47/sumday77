...
from utils.progress_formatters import build_progress_bar, format_progress_block
...
def test_protein_goal_fill_at_and_above_target(current, expected):
    assert build_progress_bar(current, 100, goal_reached_fill="🥩") == expected


def test_daily_diary_uses_protein_goal_fill_without_changing_other_macros():
...
def test_shared_dashboard_uses_protein_goal_fill_and_keeps_numeric_values():
    settings = SimpleNamespace(
        calories=2000,
        protein=100,
        fat=70,
