...
class ActivityAnalysisCopy2PromptTests(unittest.TestCase):
    def test_copy_2_prompt_uses_standard_daily_analysis_flow(self):
        import handlers.activity as activity_module
        ...
        with patch.object(
            activity_module,
            "run_daily_activity_analysis",
            side_effect=fake_run_daily_activity_analysis,
        ) as run_mock:
            asyncio.run(activity_module.analyze_activity_day_copy_2(message))
