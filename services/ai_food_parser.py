...
Ты анализируешь только текстовое описание приёма пищи.

Ответ должен быть только в формате JSON: верни строго один JSON-объект с описанной ниже структурой.
Не используй Markdown или блоки кода (включая ```json) и не добавляй никаких пояснений до или после JSON.
